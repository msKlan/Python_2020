...
class Data_sheet():
    ...
    def __init__(self, courses):
        ...
        for course in courses:
            new_course = Course(course.name, course.classroom,
                                course.teacher, course.etcs, course.grade)
...
gender = ["Male", "Female"]
imgurl = ["https://www.w3schools.com/howto/img_avatar.png", "https://www.w3schools.com/howto/img_avatar2.png",
          "https://www.w3schools.com/w3images/avatar2.png", "https://www.w3schools.com/w3images/avatar6.png"]
...
def create_random_students(no):
    ...
    students = []
    for i in range(no):
        rcs = [Course(random.choice(course_names), random.choice(classrooms), random.choice(
            teachers), random.choice(etcs), random.choice(grades)) for j in range(3)]
        ds = Data_sheet(rcs)
        students.append(Student(random.choice(names), random.choice(
            gender), ds, random.choice(imgurl)))
        ...
        for s in students:
            for c in s.data_sheet.courses:
                students_writer.writerow(
                    {'Name': s.name, 'Gender': s.gender, 'Course': c.name, 'Grade': c.grade, 'ImgUrl': s.image_url})
                # print(s.name, c.name)


def read_print_students():
    csv_file = 'random_student.csv'
    with open(csv_file, 'r', newline='') as csv_file:
        reader = csv.DictReader(csv_file)
        for row in reader:
            print(row['Name'], row['Gender'], row['Course'],
                  row['Grade'], row['ImgUrl'],)
